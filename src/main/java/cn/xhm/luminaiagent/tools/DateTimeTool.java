...
public class DateTimeTool {
    ...
    public String getCurrentDateTime(
        ...
        try {
            String formatPattern = (format == null || format.isEmpty()) ? DatePattern.NORM_DATETIME_PATTERN : format;
    ...
    public String calculateDateDifference(
        ...
        try {
            LocalDate start = LocalDate.parse(startDate, DatePattern.NORM_DATE_FORMATTER);
            LocalDate end = (endDate == null || endDate.isEmpty())
                    ? LocalDate.now()
                    : LocalDate.parse(endDate, DatePattern.NORM_DATE_FORMATTER);

            Period period = Period.between(start, end);
            long days = Duration.between(start.atStartOfDay(), end.atStartOfDay()).toDays();

            return String.format("从 %s 到 %s：\n%d年%d月%d日（总共%d天）",
                    startDate, end.format(DatePattern.NORM_DATE_FORMATTER),
    ...
    public String calculateFutureDate(
        ...
        try {
            LocalDate start = (startDate == null || startDate.isEmpty())
                    ? LocalDate.now()
                    : LocalDate.parse(startDate, DatePattern.NORM_DATE_FORMATTER);
            ...
            return String.format("从 %s 开始，%s%s%s后的日期是: %s",
                    start.format(DatePattern.NORM_DATE_FORMATTER),
                    (days != null && days != 0) ? days + "天" : "",
                    (months != null && months != 0) ? months + "个月" : "",
                    (years != null && years != 0) ? years + "年" : "",
                    result.format(DatePattern.NORM_DATE_FORMATTER));
        } catch (DateTimeParseException e) {
            return "日期格式错误，请使用yyyy-MM-dd格式：" + e.getMessage();
        } catch (Exception e) {
